...
**个人理解**：C并没有规定int类型必须是4字节，具体每个类型占用多少字节，需要根据不同的平台定义。但是C保证int至少有16位，short至少是16位，long至少为32位，long long至少是64位。
...
- 打印占位符:%d打印有符号整型，%o打印八进制，%x打印16进制，u%打印无符号整型，ld%打印长整型（long int)，lld%打印long long，%c打印字符，

- 可移植的类型，需要包含头文件`inttypes.h`，可以明确知道一个int是否是16位还是32位，例如：int16_t,uint32_t。

- C可以使用`F`后缀告诉编译器把一个浮点常量做为float看待，例如：1.23F，如果想做为long double看待，可以使用后缀`L`,例如：5.14L。  
