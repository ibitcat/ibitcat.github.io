...
- 位常用用法（假设mask=0000 0010）
	...
	- 转置：flags ^ mask,该操作会使flags中与掩码位对应为1的位被转置，对应掩码位为0的位不改变。
...
- 位移运算符
	...
	**注意**：这里涉及到逻辑位移和算数位移的区别，逻辑位移不考虑符号位，但是算术位移要考虑符号位。C语言中移位操作符实现的是逻辑左移和算术右移，但是算术左移和逻辑左移的效果相同，**算术右移和逻辑右移的效果不同**，要实现逻辑右移可将操作数强制类型转化为无符号数。
...
- 位字段由一个结构体声明建立，该结构申明为每个字段提供标签，并决定每个字段的宽度。位字段只能定义为以下3中类型：
