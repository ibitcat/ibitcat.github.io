...
7. 关于nil接口，可以查看[<font color="red">golang笔记</font>](http://domicat.me/daily-notes-golang/#nil%E5%80%BC)，这里不再赘述。
...
1. 两个相同类型的channel可以使用==运算符比较。如果两个channel引用的是相同的对象，那么比较的结果为真。一个channel也可以和nil进行比较。channel的零值也是`nil`。
