...
- 第9到12个字节：表示确认号，也是相对序号（三次握手），即告诉对方我期望收到你的下一个报文段的序号值。
...
- 小明给了小红一个苹果（SEQ=苹果1），用来搭讪，小明：“给你一个苹果，可好吃了”
- 小红吃了一个苹果后，觉得很好吃，于是给了小明一个梨子作为回礼（SEQ=梨子1），但是小红还想继续吃，就撒娇说：“人家还想吃苹果”（ACK=苹果2）
- 小明一听，哟，有戏啊，于是就把最后的一个苹果给了小红（SEQ=苹果2），同时又跟小红说：“要不你把最后那个梨子也给我吧”（ACK=梨子2）
...
- 小明（客户方）还了一个梨子给小红，并且提出了分手
- 小红答应了，也还了一个苹果给小明，并且要求把最后的例子也换回来
- 小明在之前吃了一个梨子，发现没有梨子了，就假装没反应，心里寻思着去哪里弄个梨回来，这时候小红又发来通告，“赶紧还我例子，还了赶紧分手”
- 小明最终搞搞一个梨子，并还给了小红
...
加入要发送一批TCP报文，每个报文的SN（Sequence number）分别为：1，235，790,1500，当发送790这个报文的时候，对方通过校验和检测，发现数据错误，就会返回一个带ACK的报文，要求发送方重新发送一次790，这就是TCP重传确认机制。
